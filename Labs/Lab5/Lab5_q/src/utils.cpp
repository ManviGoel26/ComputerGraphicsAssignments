...
void showOptionsDialog(unsigned  int &program, unsigned  int &program2, float &anglex, float &angley, float &anglez, float matrix[], float matrix2[]){
        ImGui::Begin("Window1");
        double current_seconds = glfwGetTime();
        ImGui::Text("Time: (current_seconds) %f", current_seconds);
        if(ImGui::SliderFloat("Angle z(radians)", &anglez, 0.0f, 3.14f)){
                std::cout << "z angle : " << anglez << std::endl;
                int mat_loc = glGetUniformLocation(program, "vModel");
                glm::mat4 rotate_mat = glm::rotate(glm::make_mat4(matrix), anglez, glm::vec3(0,0,1));
                glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));

                // mat_loc = glGetUniformLocation(program2, "vModel");
                // rotate_mat = glm::rotate(glm::make_mat4(matrix2), anglez, glm::vec3(0,0,1));
                // glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));

        };

        if(ImGui::SliderFloat("Angle y(radians)", &angley, 0.0f, 3.14f)){
                std::cout << "y angle : " << angley << std::endl;
                int mat_loc = glGetUniformLocation(program, "vModel");
                glm::mat4 rotate_mat = glm::rotate(glm::make_mat4(matrix), angley, glm::vec3(0,1,0));
                glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));

                // mat_loc = glGetUniformLocation(program2, "vModel");
                // rotate_mat = glm::rotate(glm::make_mat4(matrix2), angley, glm::vec3(0,1,0));
                // glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));

        };
        if(ImGui::SliderFloat("Angle x(radians)", &anglex, 0.0f, 3.14f)){
                std::cout << "x angle : "<<anglex << std::endl;
                int mat_loc = glGetUniformLocation(program, "vModel");
                glm::mat4 rotate_mat = glm::rotate(glm::make_mat4(matrix), anglex, glm::vec3(1,0,0));
                glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));

                // mat_loc = glGetUniformLocation(program2, "vModel");
                // rotate_mat = glm::rotate(glm::make_mat4(matrix2), anglex, glm::vec3(1,0,0));
                // glUniformMatrix4fv(mat_loc, 1, GL_FALSE, (&rotate_mat[0][0]));
